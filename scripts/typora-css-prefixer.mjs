...
const assetUrlPattern = /url\(\s*(['"]?)([^'")]+)\1\s*\)/g;
const publicAssetRoot = 'public/typora-assets';
const copiedAssets = [];
const missingAssets = [];
...
const isExternalAssetUrl = (url) => /^(?:data:|https?:|asset:|\/|#)/i.test(url);
const canResolveRelativeRemoteAsset = (sourceUrl) => Boolean(sourceUrl && /^https?:/i.test(sourceUrl) && /\.css(?:$|[?#])/i.test(sourceUrl));

const splitAssetUrl = (url) => {
  const suffixStart = url.search(/[?#]/);
  if (suffixStart === -1) return { pathPart: url, suffix: '' };
  return {
...
const normalizeAssetPublicPath = (assetPath) => {
  const normalized = posixPath.normalize(assetPath.replaceAll('\\', '/'));
  return normalized
    .replace(/^(\.\.\/)+/, (match) => match.split('../').filter(Boolean).map(() => '_up_').join('/'))
    .replace(/^\.\//, '')
    .replace(/^\/+/, '');
};
...
const inertDataUrlFor = (assetPath) => {
  const extension = assetPath.split('.').pop()?.toLowerCase() ?? '';
  if (extension === 'svg') return 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg"/%3E';
  if (['png', 'jpg', 'jpeg', 'gif', 'webp', 'avif'].includes(extension)) return 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==';
  if (extension === 'woff2') return 'data:font/woff2;base64,';
  if (extension === 'woff') return 'data:font/woff;base64,';
  if (extension === 'ttf') return 'data:font/ttf;base64,';
  if (extension === 'otf') return 'data:font/otf;base64,';
  return 'data:application/octet-stream;base64,';
};

const copyThemeAsset = async (theme, assetBaseDir, assetPath, suffix) => {
  const resolvedAssetPath = resolve(assetBaseDir, assetPath);
  if (!(await fileExists(resolvedAssetPath))) return null;

  const publicPath = normalizeAssetPublicPath(assetPath);
  const publicOutputPath = join(process.cwd(), publicAssetRoot, theme.id, publicPath);
  await mkdir(dirname(publicOutputPath), { recursive: true });
  await copyFile(resolvedAssetPath, publicOutputPath);
  copiedAssets.push({
    themeId: theme.id,
    source: resolvedAssetPath,
    publicUrl: `/typora-assets/${theme.id}/${publicPath}${suffix}`
  });
  return `/typora-assets/${theme.id}/${publicPath}${suffix}`;
};

const prepareCssAssets = async (css, theme) => {
  const rawCssPath = join(process.cwd(), rawPathFor(theme));
  const assetBaseDir = theme.assetBaseDir
    ? join(process.cwd(), theme.assetBaseDir)
    : dirname(rawCssPath);
  const replacements = [];
  ...
  for (const match of css.matchAll(assetUrlPattern)) {
    ...
    if (!trimmed || isExternalAssetUrl(trimmed)) continue;

    const { pathPart, suffix } = splitAssetUrl(trimmed);
    if (!pathPart) continue;

    const copiedUrl = await copyThemeAsset(theme, assetBaseDir, pathPart, suffix);
    if (copiedUrl) {
      replacements.push({ from: fullMatch, to: `url(${quote}${copiedUrl}${quote})` });
      continue;
    }

    if (canResolveRelativeRemoteAsset(theme.sourceUrl)) {
      replacements.push({ from: fullMatch, to: `url(${quote}${new URL(trimmed, theme.sourceUrl).href}${quote})` });
      continue;
    }

    const missingUrl = inertDataUrlFor(pathPart);
    missingAssets.push({
      themeId: theme.id,
      url: trimmed,
      expectedFrom: assetBaseDir,
      replacement: missingUrl
    });
    replacements.push({ from: fullMatch, to: `url(${quote}${missingUrl}${quote})` });
  }

  return replacements.reduce((currentCss, replacement) => currentCss.replace(replacement.from, replacement.to), css);
};
...
for (const theme of themes) {
  ...
  const cssForScoping = await prepareCssAssets(rawCss, theme);
...
const manifest = themes.map((theme) => ({
  ...
  ...(theme.assetBaseDir ? { assetBaseDir: theme.assetBaseDir } : {})
}));
...
if (copiedAssets.length) {
  console.log(`Copied ${copiedAssets.length} Typora theme asset${copiedAssets.length === 1 ? '' : 's'} into /typora-assets/.`);
}

if (missingAssets.length) {
  console.warn(`Missing ${missingAssets.length} Typora theme asset${missingAssets.length === 1 ? '' : 's'}; rewritten to inert data URLs so Vite does not silently bind them to app CSS.`);
  for (const asset of missingAssets) {
    console.warn(`  - ${asset.themeId}: ${asset.url} (expected under ${asset.expectedFrom})`);
  }
}
