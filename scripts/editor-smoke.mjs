...
const ensureToolbarVisible = async () => {
  ...
  if (!text.trim()) await page.keyboard.type('toolbar probe');
...
checks.toolbarIndentInSwiss = listHtml.includes('<ul') && (listHtml.match(/<ul/g) ?? []).length >= 2 && listHtml.includes('child');
await page.keyboard.press('Shift+Tab');
